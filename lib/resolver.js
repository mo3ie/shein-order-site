...
export async function pollResolveJob(jobId) {
  const { body } = await call(`/jobs/${encodeURIComponent(jobId)}`, { method: "GET" });
  if (body.status === "pending") return { status: "pending", elapsedMs: body.elapsedMs };
  if (body.status === "error") {
    const code = body.error?.code || "RESOLVER_FAILED";
    throw new ResolverError(CUSTOMER_MESSAGE[code] ? code : "RESOLVER_FAILED", body.error?.message);
  }
  return { status: "done", ...shapeResult(body) };
}
 ...
 */
export async function resolveSharedCart(shareUrl, { maxWaitMs = TIMEOUT_MS } = {}) {
  const { jobId } = await startResolveJob(shareUrl);
