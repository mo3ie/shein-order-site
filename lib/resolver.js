...
const RESOLVER_URL_ENV = process.env.RESOLVER_URL || "http://localhost:8787";

/**
 * عنوان خدمة التسعير: من قاعدة البيانات أولاً، ثم من المتغيّر البيئي.
 *
 * العنوان كان مثبّتًا في متغيّر بيئة على Vercel، فحين انقطع النفق الذي يحمله
 * صار تغييره يتطلّب نشرًا جديدًا — والخدمة واقفة طوال ذلك. صار العنوان صفًّا في
 * الإعدادات يمكن للخادم نفسه أن يسجّله عند إقلاعه، فيتعافى المسار وحده بلا نشر.
 * القراءة مخبّأة 30 ثانية حتى لا يُسأل الجدول مع كل طلب.
 */
let cachedUrl = null;
let cachedAt = 0;
export async function resolverUrl() {
  if (cachedUrl && Date.now() - cachedAt < 30000) return cachedUrl;
  try {
    const { supabaseAdmin } = await import("@/lib/supabaseAdmin");
    const { data } = await supabaseAdmin.from("settings").select("resolver_url").eq("id", 1).single();
    const url = String(data?.resolver_url || "").trim();
    cachedUrl = /^https?:\/\//.test(url) ? url.replace(/\/+$/, "") : RESOLVER_URL_ENV;
  } catch {
    cachedUrl = RESOLVER_URL_ENV;
  }
  cachedAt = Date.now();
  return cachedUrl;
}
/** يُسقط العنوان المخبّأ فورًا، فيُقرأ من الإعدادات في النداء التالي. */
export function invalidateResolverUrl() { cachedUrl = null; cachedAt = 0; }
...
/**
 * نداء واحد للخدمة — مع احتمال الومضة.
 *
 * النفق يسقط أحيانًا طلبًا واحدًا ويعيد صفحة 502 من كلاودفلير، ثم يعود سليمًا
 * في الثانية التالية: مراقبُ الصحة لا يلمح هذه الومضات (قِيست ٢٠/٢٠ ناجحة من
 * الموقع في اللحظة نفسها التي فشل فيها زبون)، لكن الزبون يدفع ثمنها كاملاً —
 * "خدمة التسعير غير متاحة" على محاولة كان يكفيها انتظار ثانية.
 *
 * فنعيد المحاولة عند أعطال البوّابة وحدها: خطأ شبكة، أو ردّ ليس JSON بحالة 5xx.
 * أما ردّ الخدمة الصحيح — ولو كان رفضًا — فيمرّ كما هو بلا تكرار. وإعادة إرسال
 * POST /jobs آمنة لأن الخدمة تُوحّد المهامّ بمفتاح الرابط: المحاولة الثانية
 * تلتحق بالأولى ولا تبدأ قياسًا ثانيًا على الحساب نفسه.
 */
const CALL_ATTEMPTS = 3;
const CALL_BACKOFF_MS = [1000, 2500];

async function callOnce(path, init, timeoutMs) {
  ...
  try {
    ...
    try {
      ...
      // بوّابة تردّ صفحة HTML لا JSON: عطل نفق، لا جواب خدمة.
      const err = new ResolverError("RESOLVER_UNREACHABLE", `non-JSON from resolver (HTTP ${res.status})`);
      err.gateway = true;
      throw err;
    }
    return { res, body };
    ...
    const err = new ResolverError(e.name === "AbortError" ? "TIMEOUT" : "RESOLVER_UNREACHABLE", e.message);
    // المهلة لا تُعاد: القياس قد يكون بدأ فعلاً وطال. أما انقطاع الشبكة فيُعاد.
    err.gateway = e.name !== "AbortError";
    throw err;
  } finally {
    clearTimeout(timer);
  }
...
async function call(path, init = {}, timeoutMs = 30000) {
  ...
  for (let attempt = 0; attempt < CALL_ATTEMPTS; attempt++) {
    try {
      return await callOnce(path, init, timeoutMs);
    } catch (e) {
      last = e;
      if (!e.gateway) throw e;
      // العنوان قد يكون تبدّل للتوّ (بدّل المراقبُ النفق): لا نتشبّث بالمخبّأ.
      invalidateResolverUrl();
      if (attempt < CALL_ATTEMPTS - 1) {
        console.warn(`[resolver] ${path} gateway failure (${e.detail || e.message}) — retry ${attempt + 1}`);
        await new Promise((r) => setTimeout(r, CALL_BACKOFF_MS[attempt]));
      }
    }
  }
  throw last;
...
function shapeResult(body) {
  ...
  // A total far below the lines it is made of is a misread, not a bargain.
  //
  // The checkout summary is read off a screen, and a partial read (a row still
  // loading, a sheet half open) can return a retail line belonging to only part
  // of the cart — once quoting $116 for a cart whose own lines added past
  // $1,000. SHEIN's promotions do cut the total, but never to a third of it, so
  // a total under 55% of the lines is refused instead of quoted: the customer
  // is asked to try again rather than sold to below cost.
  const lines = Array.isArray(body.items) ? body.items : [];
  const linesUsd = lines.reduce((sum, i) => {
    const unit = Number(i.unitRetailUsd ?? i.unitSaleUsd ?? 0);
    const qty = Number(i.quantity ?? 1) || 1;
    return sum + (Number.isFinite(unit) ? unit * qty : 0);
  }, 0);
  if (linesUsd > 0 && price < linesUsd * 0.55) {
    throw new ResolverError(
      "PRICE_NOT_FOUND",
      `total ${price} is below 55% of the cart's own lines (${linesUsd.toFixed(2)})`
    );
  }
  return {
    ...
    // قائمة ناقصة لا تُبنى عليها كميات.
    itemsComplete: body.itemsComplete !== false,
