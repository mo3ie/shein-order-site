      ...
      <div className="form-inner" style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ background: GRAD_HEAD, color: "#fff", padding: "18px 20px 24px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative" }}>
            ...
            <a href={user ? "/account" : "/login"} style={{ width: 30, height: 30, borderRadius: "50%", background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13.5, fontWeight: 700, color: "#fff", textDecoration: "none" }}>
          ...
          <div style={{ marginTop: 22, position: "relative" }}>
            <div style={{ fontSize: 13.5, opacity: 0.8, marginBottom: 4 }}>{t("رصيد محفظتك")}</div>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 12, flexWrap: "wrap" }}>
              <span style={{ fontSize: 12.5, background: "rgba(255,255,255,0.16)", borderRadius: 20, padding: "4px 11px" }}>{t("الدفع منها بلا رمز تحقق")}</span>
              <span style={{ fontSize: 12.5, background: "rgba(255,255,255,0.16)", borderRadius: 20, padding: "4px 11px" }}>{t("محفظة شي إن وحدها")}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 13 }}>
          ...
          {user === null ? (
            <div style={{ background: CARD, borderRadius: 18, padding: "48px 20px", textAlign: "center", boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
              ...
              <p style={{ fontSize: 14, color: FAINT, marginBottom: 20 }}>{t("الرصيد مرتبط بحسابك")}</p>
              <a href="/login" style={{ display: "inline-block", padding: "13px 28px", borderRadius: 14, background: GRAD_HEAD, color: "#fff", fontWeight: 800, fontSize: 15.5, textDecoration: "none", boxShadow: "0 6px 18px rgba(124,58,237,0.28)" }}>{t("تسجيل الدخول")}</a>
            </div>
          ) : (<>

            <div style={{ background: CARD, borderRadius: 18, padding: 15, boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
              <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 9 }}>{t("كيف تُستعمل المحفظة")}</div>
              ...
              <a href="/" style={{ display: "block", textAlign: "center", marginTop: 12, padding: 13, borderRadius: 13, background: CHIP, color: PRIMARY, fontSize: 14.5, fontWeight: 800, textDecoration: "none" }}>{t("ابدأ طلباً وادفع من المحفظة")}</a>
            </div>

            <div style={{ fontSize: 15.5, fontWeight: 800, marginTop: 2 }}>{t("حركات الرصيد")}</div>

            {loading ? (
              <div style={{ textAlign: "center", padding: 30, color: FAINT, fontSize: 14.5 }}>{t("جاري التحميل...")}</div>
            ) : txs.length === 0 ? (
              <div style={{ background: CARD, borderRadius: 18, padding: "40px 20px", textAlign: "center", boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
                ...
                <p style={{ fontSize: 14, color: FAINT }}>{t("سيظهر هنا كل شحن ودفع من المحفظة")}</p>
              </div>
            ) : (
                  ...
                  return (
                    <div key={tx.id} style={{ background: CARD, borderRadius: 16, padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14.5, fontWeight: 800 }}>{isIn ? t("شحن رصيد") : t("دفع طلب")}</div>
                      ...
                      <div style={{ textAlign: "left", flexShrink: 0 }}>
                        <div style={{ fontSize: 15.5, fontWeight: 900, color: isIn ? "var(--t-green-ink)" : INK }}>
                          {isIn ? "+" : "−"}{Number(tx.amount).toFixed(0)}
                        </div>
                        <div style={{ fontSize: 12, color: FAINT }}>{t("د.ل")}</div>
                      </div>
                    </div>
                  );
                })}
