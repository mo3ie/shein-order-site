      ...
      <div className="form-inner" style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <div style={{ background: GRAD_HEAD, color: "#fff", padding: "18px 20px 24px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ marginTop: 22, position: "relative" }}>
            <div style={{ fontSize: 12, opacity: 0.8, marginBottom: 4 }}>رصيد محفظتك</div>
        ...
        <div style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 13 }}>
          ...
          {user === null ? (
            <div style={{ background: CARD, borderRadius: 18, padding: "48px 20px", textAlign: "center", boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
              <p style={{ fontWeight: 800, marginBottom: 6 }}>سجّل دخولك لعرض محفظتك</p>
              <p style={{ fontSize: 12.5, color: FAINT, marginBottom: 20 }}>الرصيد مرتبط بحسابك</p>
              <a href="/login" style={{ display: "inline-block", padding: "13px 28px", borderRadius: 14, background: GRAD_HEAD, color: "#fff", fontWeight: 800, fontSize: 14, textDecoration: "none", boxShadow: "0 6px 18px rgba(124,58,237,0.28)" }}>
                تسجيل الدخول
              </a>
            </div>
          ) : (<>
            ...
            <div style={{ background: CARD, borderRadius: 18, padding: 15, boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
              <div style={{ fontSize: 12.5, fontWeight: 800, marginBottom: 9 }}>كيف تُستعمل المحفظة</div>
              ...
              <a href="/" style={{ display: "block", textAlign: "center", marginTop: 12, padding: 13, borderRadius: 13, background: CHIP, color: PRIMARY, fontSize: 13, fontWeight: 800, textDecoration: "none" }}>
                ابدأ طلباً وادفع من المحفظة
              </a>
            </div>

            <div style={{ fontSize: 14, fontWeight: 800, marginTop: 2 }}>حركات الرصيد</div>

            {loading ? (
              <div style={{ textAlign: "center", padding: 30, color: FAINT, fontSize: 13 }}>جاري التحميل...</div>
            ) : txs.length === 0 ? (
              <div style={{ background: CARD, borderRadius: 18, padding: "40px 20px", textAlign: "center", boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
                <p style={{ fontWeight: 800, marginBottom: 6 }}>لا حركات بعد</p>
                <p style={{ fontSize: 12.5, color: FAINT }}>سيظهر هنا كل شحن ودفع من المحفظة</p>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                {txs.map((t) => {
                  const isIn = t.type === "topup";
                  return (
                    <div key={t.id} style={{ background: CARD, borderRadius: 16, padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 2px 10px rgba(22,19,31,0.05)" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 800 }}>{isIn ? "شحن رصيد" : "دفع طلب"}</div>
                        <div style={{ fontSize: 11, color: FAINT, marginTop: 2 }}>
                          {fmtDate(t.created_at)}
                          {t.method ? ` · ${METHOD[t.method] || t.method}` : ""}
                        </div>
                      </div>
                      <div style={{ textAlign: "left", flexShrink: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 900, color: isIn ? "var(--t-green-ink)" : INK }}>
                          {isIn ? "+" : "−"}{Number(t.amount).toFixed(0)}
                        </div>
                        <div style={{ fontSize: 10.5, color: FAINT }}>د.ل</div>
                      </div>
                    </div>
                  );
                })}
